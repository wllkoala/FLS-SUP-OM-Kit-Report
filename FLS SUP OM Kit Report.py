...
# 生成原始df
df_om = pd.read_excel(
    om_kit_report, sheet_name="FLS SUP OM Kit Report", skiprows=1)
...
# 将df内容用‘|’进行连接
sort_completeunit = []
sort_completeunit.append("|".join([str(item)
                                   for item in df_completeunit.iloc[:, 0]]))
# 打开Sales order data report
sales_order_data_report = filedialog.askopenfilename(
    title="FLS CN Sales Order Data Report by Creation Date", filetypes=[("All files", "*")],)
df_so = pd.read_excel(sales_order_data_report,
                      sheet_name="FLS CN Sales Order Data Report ")
# 原始df_om去除重复的End Item
df2_om = df_om.drop_duplicates(subset=["End Item"])
# 提取部分列
df2_om = df2_om[["Project Num", "Supplymake Planner Code", "End So Num", "Customer Name",
                 "End So Line", "End So Scheduled Date", "End Item Description", "End Item", ]]
# 提取部分列
df2_so = df_so[["Order Number", "Line Number", "Item Number",
                "Product Type", "Pump Size", "Ordered Quantity", ]]
# 按照OM Kit对列重命名
df2_so = df2_so.rename(columns={"Order Number": "End So Num", "Line Number": "End So Line",
                                "Item Number": "End Item", "Product Type": "Category", "Pump Size": "Pump Family", })
# 去除重复行
df2_so = df2_so.drop_duplicates(
    subset=["End So Num", "End So Line", "End Item", "Category", "Pump Family"])
# 将df按照关键列合并
df_summary = pd.merge(df2_om, df2_so, on=[
                      "End So Num", "End So Line", "End Item"], how="inner")
# 插入列‘光泵齐套情况’、‘整泵齐套情况’
...
for i in range(len(df_summary)):
    ...
    index_list = list(
        df1[df1["Supplymake Item"].str.contains(sort_barepump[0])].index)
    material_status = []
    for n in index_list:
